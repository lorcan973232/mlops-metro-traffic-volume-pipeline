...
def read_json(path: str | Path) -> dict[str, Any]:
    """Read a JSON report file and report missing or invalid files clearly."""
    file_path = Path(path)
...
def latest_sha() -> str:
    """Return the current commit SHA used to tie reports to this checkout."""
...
def command_results() -> dict[str, Any]:
    """Summarise local checks without rerunning every heavy command."""
    return {
        "generated_at": datetime.now(UTC).replace(microsecond=0).isoformat(),
        "note": (
            "This file summarises reports generated by the dedicated verification "
...
def write_summary(report: dict[str, Any]) -> None:
    """Write a stable human summary that points to generated current reports."""
    lines = [
        "# Final Project Readiness Summary",
        "",
        "This committed file is a stable description of the readiness reports.",
        "It deliberately avoids hard-coded commit SHAs and workflow conclusions.",
        "",
        "Current SHA-specific reports are generated by `python scripts/final_readiness_check.py`",
...
def main() -> None:
    """Generate all final-readiness report files for this checkout."""
